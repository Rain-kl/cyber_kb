...
class TikaDocumentConvertor(DocumentConvertor):
    ...
    def extract_text(self, file_path: str) -> str:
        ...
        # 准备发送到 Tika 服务器的请求
        headers = {"Accept": "text/plain"}  # 请求纯文本输出

        with open(file_path, "rb") as file:
            # 发送文件到 Tika 服务器进行解析
            response = requests.put(
                f"{self.tika_server_url}/tika", headers=headers, data=file
            )
    ...
    def extract_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        # 准备发送到 Tika 服务器的请求
        headers = {"Accept": "application/json"}  # 请求 JSON 格式的元数据

        with open(file_path, "rb") as file:
            # 发送文件到 Tika 服务器进行元数据提取
            response = requests.put(
                f"{self.tika_server_url}/meta", headers=headers, data=file
            )
            response.raise_for_status()
